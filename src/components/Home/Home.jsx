...
const Home = () => {
    ...
    return (
        <section className="home">
            <div className={light ? "home__div1" : "home__div1__dark"}></div>
            <div className={ light ? 'home__div2' : 'home__div2__dark'}></div>
            <section className="home__fixed">
                <div className="home__fixed__header">
                    <h1>TO DO</h1>
                    <i><img src={light ? moon : sun} alt="icon moon" onClick={handleTheme}/></i>
                </div>
                <div className={ light ? "home__fixed__input" : "home__fixed__input__dark"}>
                    <div></div>
                    <input
                        type="text"
                ...
                <div className={light ? "home__fixed__list" : "home__fixed__list__dark"}>
                    <div className="home__fixed__list__itemsContainer">
                        {
                            todo.filter((item) => {
                                ...
                                <div key={index} className={`home__fixed__list__itemsContainer__item${item.status === 'active' ? '__active' : '__completed'}`}>
                                    ...
                                    <p>{item.to_do}</p>
                                    <i onClick={() => handleDeleteToDo(item)}><img src={x} alt="close icon" className="x"/></i>
                                </div>
                            ))
                        }
                    </div>
                ...
                <p className="home__fixed__p">Drag and drop to reorder list <Link to={"/"}> Log out</Link></p>
            </section>
        </section>
    )
}
